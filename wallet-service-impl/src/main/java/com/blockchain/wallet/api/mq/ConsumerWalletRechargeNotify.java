 ...
 */
@Slf4j
public class ConsumerWalletRechargeNotify implements ChannelAwareMessageListener {

    private final Jackson2JsonMessageConverter converter;
    private final WalletAccountBusiness bussiness;
    private final walletBussinessMqMsgService mqMsgService;

    public ConsumerWalletRechargeNotify(Jackson2JsonMessageConverter converter, WalletAccountBusiness bussiness, walletBussinessMqMsgService mqMsgService) {
        this.converter = converter;
        this.bussiness = bussiness;
        this.mqMsgService = mqMsgService;
    }

    private walletBussinessMqMsgPOWithBLOBs generate(WalletUsdtMsgDTO dto){
        walletBussinessMqMsgPOWithBLOBs po = new walletBussinessMqMsgPOWithBLOBs();
        po.setMsgContent(JSON.toJsonString(dto));
        po.setBussinessType("RechargeNotify");
        po.setMsgClazzType(WalletUsdtMsgDTO.class.getName());
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        ...
        WalletUsdtMsgDTO dto = (WalletUsdtMsgDTO)obj;
        try{
            bussiness.rechargeNotifyBill(dto);
            ...
            if(e instanceof BizException){
                ...
                // db 异常可以尝试 重试操作3次来处理。
                MessageCounter<WalletUsdtMsgDTO> counter = MessageCounter.getInstance();
                if(counter.add(dto)){
                    Thread.sleep(1000L);
